// FROM: https://gist.github.com/ranwahle/e78a8859ff78bdd68055017fc5875fcb
...
export class TimeSpan {
    ...
    public get days(): number {
        return TimeSpan.round(this.millis / MILLIS_PER_DAY);
    }

    public get hours(): number {
        return TimeSpan.round((this.millis / MILLIS_PER_HOUR) % 24);
    }

    public get minutes(): number {
        return TimeSpan.round((this.millis / MILLIS_PER_MINUTE) % 60);
    }

    public get seconds(): number {
        return TimeSpan.round((this.millis / MILLIS_PER_SECOND) % 60);
    }

    public get milliseconds(): number {
        return TimeSpan.round(this.millis % 1000);
    }
    ...
    public static fromTime(daysOrHours: number, hoursOrMinutes: number, minutesOrSeconds: number, seconds?:
                           number, milliseconds?: number): TimeSpan {
   ...
   private static fromTimeStartingFromHours(hours: number, minutes: number, seconds: number): TimeSpan {
    ...
    public add(ts: TimeSpan): TimeSpan {
        const result = this.millis + ts.totalMilliseconds;
